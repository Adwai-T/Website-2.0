 ...
 * @param {number} y 
 * @param {string} color 
 * @param {string} font - eg. ctx.font = 'bold 48px serif'; default size is 10px;
 * @param {string} align - top, hanging, middle, alphabetic, ideographic, bottom
 */
...
export class Circle {
  constructor(center, radius, color, isSolid, thickness) {
    ...
    this.thickness =  typeof(thickness)=== 'number' ? thickness : 1;
  ...
  /**
   * Draw circle with text inside it.
   * @param {HTMLCanvasElement.context} ctx 
   * @param {string} color 
   * @param {string} text 
   ...
   * @param {string} align - top, hanging, middle, alphabetic, ideographic, bottom
   * @param {number} thickness 
   * @param {boolean} isSolid - if the circle is solid
   */
  drawWithStringInside(ctx, color, text, stringColor, font, align, thickness, isSolid) {
    color = typeof(color) === 'string' ? color : this.color;
    thickness = typeof(color) === 'number' ? thickness : this.thickness;
    this.isSolid = isSolid;

    if(this.isSolid) ctx.fillStyle = color;
    ...
    //Drawing Text
    ctx.fillStyle = typeof(stringColor) === 'string' ? stringColor : 'black';
    ctx.font = typeof(font) === 'string' ? font : 'bold 10px serif';
    ctx.textAlign = typeof(align) === 'string' ? align : 'alphabetic';
    let textDimensions = ctx.measureText(text);
    let stringDrawPostion = new Vector2i(
                                this.center.x - textDimensions.width/2, 
                                this.center.y + textDimensions.actualBoundingBoxAscent/2
    );
    ctx.fillText(text, stringDrawPostion.x, stringDrawPostion.y);
  }
}
